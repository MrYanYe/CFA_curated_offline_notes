...
| 体积 | 309 MB（1 个文件夹） | **105.5 MB**（原始未压缩）/ **80.4 MB**（压缩版） |
...
| 体积 | 309 MB | 105.5 MB 原始 / 80.4 MB 压缩 |
...
| 迁移性 | 拷整个文件夹（309 MB） | 拷 1 个文件（105.5 或 80.4 MB） |
...
点击后装载真实播放器 iframe——**有网点击即可播放**（使用 YouTube 官方隐私增强 nocookie 嵌入域，
规避“Error 153 配置错误”），框上另有“Open on YouTube”外链保底；离线时不产生任何网络请求（不会卡页面）。

**Q3 单文件版为什么 95 MB？**
原图无损内嵌（用户选定方案）：全部图片去重后约 74 MB 的 base64 去重注册表 + 305 页正文 + 字体/CSS。
若希望更小，可对图片重压缩增量替换（需在 `build_single_file.py` 增改图片处理策略）。

**Q5 图片曾经每张显示两遍，现在修好了吗？**
修好了（2026-09-06）。根因：原始站点快照里每张图都带了第二份副本（页面
模板双份渲染），离线打开时两份全部显示。修复：构建后按"每页同 src 图片只保留第一份"
去重（与在线原站实际渲染的每图 1 份一致），已集成 `build_site.py`。
实测：去重后每页图片数恰好减半（26→13），JS 关闭状态下重复统计为 **0**，
桌面/手机截图确认每张图仅显示一次。

**Q7 为什么有的文章标题图是黑色大横幅？**
那是原站自带的设计图（如 CFA L1 "Financial Reporting & Analysis" 章节横幅：黑底+金色标题，
原图即如此）。已验证在线原站同页渲染完全一致（黑块与尺寸均为原图内容），非本项目缺陷。

**Q6 单文件版里的 Notes Navigation 侧栏、视频占位、返回按钮正常吗？**
正常（2026-09-06 修复）。① 单文件版路由页注入自己的 Notes Navigation 侧栏，**当前所在目录
青色高亮**（与原站一致）；② 视频为点击播放的播放器框（见 Q2）；③ 返回按钮：退回上级页面正确显示，
退到首页时完整恢复首页内容（以前会停在当前页不动，再按就退出）。

**Q4 下载失败/图片缺失？**
镜像已覆盖 99%+ 资源：2026-09-06 通过 761 条断链 URL 逐条在线核对，其中 153 条
可下载（已补齐），其余原站反爬规则对少数旧图长期返回 403/404（**在线原站同样无法显示这些图**，
已收录于 `verify_links.py` 的已知清单）。若以后有新的 broken ref，按第六节命令重跑即可定位。
